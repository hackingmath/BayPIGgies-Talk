...
def generate(i):
    ...
    #me = cells[i-1]
    #left = numpy.roll(me, -1)
    #right = numpy.roll(me, 1)
...
npscreen = numpy.zeros((HEIGHT, WIDTH, 3), dtype=numpy.uint8) # array to blit to screen
...
while not done:
    ...
    # overwrite the red pixels on the screen with expanded version of cells
    npscreen[:,:,0] = cells[i0:i1,j0:j1].repeat(w, axis=1).repeat(w, axis=0) * 255
    pygame.surfarray.blit_array(screen, npscreen)
